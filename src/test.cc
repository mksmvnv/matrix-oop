...
TEST(EqMatrix, DiffDimension) {
    int size = 4;
    Matrix a(size, size), b(size - 1, size - 1);
    ASSERT_EQ(a.EqMatrix(b), 0);
    ASSERT_EQ(b.EqMatrix(a), 0);
}
...
TEST(Constructor, Constructor_Copy) {
    int rows = 3;
    int columns = 4;
    Matrix sample(rows, columns);
    Matrix expect(sample);
...
TEST(Complements, 1) {
    ...
    expected(2, 1) = -2;
    expected(2, 2) = 4;
    Matrix result = source.CalcComplements();
...
TEST(Complements, 2d) {
    Matrix source(2, 2), expected(2, 2);
    source(0, 0) = 3;
    source(0, 1) = 1;
    source(1, 0) = 20;
    source(1, 1) = -2;
...
TEST(Overload, Sum) {
    Matrix source1(3, 3), source2(3, 3), expected(3, 3);
    source1(0, 0) = 352.412;
    source1(1, 0) = 1;
    source1(2, 0) = 1;
...
TEST(SumMatrix, 1dMatrix) {
    Matrix source1, source2, expected;
    source1(0, 0) = 352.412;
...
TEST(MulMatrix, DifferentDimension) {
    Matrix source1(3, 3), source2(3, 1), expected(3, 1);
    source1(0, 0) = 5;
    source1(0, 1) = 8;
    source1(0, 2) = -4;
    source1(1, 0) = 6;
...
TEST(MulMatrix, BigMatrices) {
    Matrix source1(2, 4), source2(4, 6), expected(2, 6);
    source1(0, 0) = 9;
    source1(0, 1) = 9;
    source1(0, 2) = 2;
    source1(0, 3) = 8;
...
TEST(MulMatrix, EmptyMatrices) {
    Matrix source1(19, 9), source2(9, 14), expected(19, 14);
    source1.MulMatrix(source2);
    ASSERT_EQ(source1.EqMatrix(expected), 1);
}
...
TEST(Transpose, 3dMatrix) {
    Matrix source(3, 3), expected(3, 3);
    source(0, 0) = 352.412;
    source(1, 0) = 1;
    source(2, 0) = 1;
...
TEST(Transpose, 3x4Matrix) {
    Matrix source(3, 4), expected(4, 3);
    source(0, 0) = 352.412;
    source(1, 0) = 1;
    source(2, 0) = 1;
    source(0, 1) = 1234.8234;
...
TEST(Transpose, 4x3Matrix) {
    Matrix source(4, 3), expected(3, 4);
    source(0, 0) = 352.412;
    source(1, 0) = 1;
    source(2, 0) = 1;
...
TEST(MulMatrix, Size_error) {
    Matrix source1(2, 2), source2(5, 5);
...
TEST(Constructors, WrongIndex) {
    EXPECT_THROW(Matrix source(-1, -1), std::bad_array_new_length);
}
...
TEST(SumMatrix, DiffSize) {
    Matrix source1(3, 2), source2(2, 3);
...
TEST(Constructors, Move) {
    ...
    source(2, 2) = -99;
    source(1, 1) = 13;
    Matrix moved_matrix(std::move(source));
...
TEST(InverseMatrix, BigMatrix) {
    int size = 8;
    Matrix source(size, size), expected(size, size);
    source(0, 0) = 2;
    source(0, 1) = 8;
    source(0, 2) = 7;
    source(0, 3) = 4;
    ...
    Matrix result = source.InverseMatrix();
    ASSERT_EQ(result.EqMatrix(expected), 1);
}
